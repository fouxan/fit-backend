...
class ExerciseService:
    ...
    def create_exercise(
        ...
        category = (
            ...
            .filter(ExerciseCategory.id == exercise.category_id)
            .first()
        )
        if not category:
        ...
        muscle_groups = (
            ...
            .filter(MuscleGroup.id.in_(exercise.muscle_group_ids))
            .all()
        )
        if len(muscle_groups) != len(exercise.muscle_group_ids):
            raise HTTPException(
                status_code=404, detail="One or more muscle groups not found"
            )

        # Verify equipment exists
        equipment = (
            db.query(Equipment).filter(Equipment.id.in_(exercise.equipment_ids)).all()
        )
        if len(equipment) != len(exercise.equipment_ids):
            raise HTTPException(
                status_code=404, detail="One or more equipment items not found"
            )
    ...
    def update_exercise(
        ...
        if db_exercise.is_custom and db_exercise.created_by_id != user.id:
            raise HTTPException(
                status_code=403, detail="Not authorized to update this exercise"
            )

        # If it's a system exercise, only admins can update
        if not db_exercise.is_custom and not user.is_superuser:
            raise HTTPException(
                status_code=403, detail="Not authorized to update system exercises"
            )
        ...
        if "muscle_group_ids" in update_data:
            muscle_groups = (
                db.query(MuscleGroup)
                .filter(MuscleGroup.id.in_(update_data["muscle_group_ids"]))
                .all()
            )
        ...
        if "equipment_ids" in update_data:
            equipment = (
                db.query(Equipment)
                .filter(Equipment.id.in_(update_data["equipment_ids"]))
                .all()
            )
    ...
    def delete_exercise(db: Session, exercise_id: str, user: User) -> None:
        ...
        if exercise.is_custom and exercise.created_by_id != user.id:
            raise HTTPException(
                status_code=403, detail="Not authorized to delete this exercise"
            )

        # If it's a system exercise, only admins can delete
        if not exercise.is_custom and not user.is_superuser:
            raise HTTPException(
                status_code=403, detail="Not authorized to delete system exercises"
            )
